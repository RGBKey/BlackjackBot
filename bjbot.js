...
var version = '0.1.5',
	...
	owner = '55806',
	chatAllowance = 4,
	Blackjack = require('./BJ.js'),
	GAME;

//Read all files

//Save all the files every 5 minutes to protect against data loss
setInterval(saveFiles, 1000 * 60 * 5, (success) => {
	if(!success){
		Util.important('Error saving files');
	} else {
		Util.log('Saved files');
	}
});
...
function show_help() {
    Util.log('type to chat, or (.b)et, (.c)hance, (.d)eposit, (.f)latbet, (.h)i, (.l)o, (.m)artingale (.n)ame (.p)ayout, (.s)take, (.t)oggle (.w)ithdraw (.help)');
    Util.log('hit return on its own to repeat last line');
}
...
function run_bot(cookie) {
    ...
    socket.on('tip', function(from, name, amount, comment, ignored) {
        ...
        Util.log('TIP: you received ' + amount + ' from (' + from + ') <' + name + '>' + (comment ? ' [' + comment + ']' : ''));
    });
    ...
    socket.on('init', function(data) {
        ...
        if (!inits++) {
            ...
            Util.log('connected as (' + uid + ') <' + data.name + '>');
            show_news(data.news);
    ...
    socket.on('set_hash', function(hash) {
        Util.log('INFO:', 'server requested that we reconnect...');
        socket.close();
        run_bot(cookie);
    });
    ...
    socket.on('chat', function(txt, date) {
		var chatRegex = /^\((\d+)\) <@?(.*?)> (.*)$/;
		var pmRegex = /^\[ \((\d+)\) <@?(.*?)> → \(\d+\) <.*?> \] (.*)$/;
		var result = chatRegex.exec(txt);
		var pmResult = pmRegex.exec(txt);
		if(pmResult){
			var id = pmResult[1];
			var username = pmResult[2];
			var message = pmResult[3];
			var cmd = message.split(' ')[0];
			console.log(`PM (${id}) <${username}> ${message}`);
			chatbotCommands(cmd, message, id, username, true);
		}
		if(result){
			var id = result[1];
			...
			var cmd = message.split(' ')[0];
			chatbotCommands(cmd, message, id, username, false);
		}
    });
...
function pm(id, message){
	socket.emit('chat', csrf, `/msg ${id} ${message}`);
}

function chatbotCommands(cmd, message, id, username, isPM){
	cmd = cmd.toLowerCase();
	...
	if(id == owner){
		switch(cmd){
			case '!new-game':
				GAME = new Blackjack(8,0,0,(msg)=>{
					say(`/me ${msg}`);
				}, (id, msg)=>{
					say(`/msg ${id} ${msg}`);
				});
				pm(id, 'New game initialized');
				break;
			case '!add-player':
				GAME.addPlayer(args[0], args[1], 1);
				pm(id, 'Added player');
				break;
			case '!start-game':
				GAME.playGame(() => {
	...
	if(GAME && GAME.playerId && GAME.playerId == id){
		switch(GAME.waitingOn){
			case 'hit':
